...
m4=(m1+m2+m3)*0.03;
kesai4=0.05;
M=[m1,0,0,0;0,m2,0,0;0,0,m3,0;0,0,0,m4]; %输入质量矩阵
K=[k1+k2,-k2,0,0;-k2,k2+k3,-k3,0;0,-k3,k3+k4,-k4;0,0,-k4,k4]; %输入刚度矩阵
kesai=[0.05;0.05];  %输入前两阶和TMD阻尼比
kesaiTMD=0.05
n=length(M);  %维数
...
c=dam_a(1)*M+dam_a(2)*K;  %求出瑞利阻尼矩阵(非必要）
for j=1:n
    kesai(j)=dam_a(1)/2/eigvalue(j)+dam_a(2)*eigvalue(j)/2;  %已知前两阶阻尼比，求所有阻尼比(后两阶为非精确值，TMD的阻尼比可用精确值替换）
end
kesai(4)=kesaiTMD;%TMD阻尼比精确值替换非精确值
kesai%为什么求出后为列向量请看第34行
%=====================地震作用下的结构位移计算(三种方法)=======================
...
beita=2.*f.*pi./wd;%频率比(三阶）
for l=1:n
fre_res(l,:)=(1/KG(l))*((1-beita(l,:).^2)+i*2*kesai(l)*beita(l,:)).^-1;%频响函数也是负正频率对应的值(分三阶)
end
